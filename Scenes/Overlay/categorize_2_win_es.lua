...
local function audioPlayer()
	activeVoice("Audio/voice/level_2_finish.wav")
end
...
local function onContinue() --continue menu handler 
	...
	composer.gotoScene( "GameMode.categorize_3_es", { time=300, effect="zoomInOutFade" } )
end
-----------------------------------------
-- Scenes
-----------------------------------------
function scene:create( event )
	...
	local levelText = display.newText( mainGroup, "Nivel 2 completado!", display.contentCenterX, display.contentCenterY - 10, "Fonts/FORTE.TTF", 40 )
...
function scene:hide( event )
	...
	elseif( phase == "did" )then
		...
		composer.hideOverlay( "Scenes.Overlay.categorize_2_win_es" )		--Remove scene when scene goes away	end
	end
end
--scenes listeners
