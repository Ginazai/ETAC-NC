...
local function onContinue() --continue menu handler 
	...
	composer.gotoScene( "GameMode.categorize_2_es", { time=300, effect="zoomInOutFade" } )
end
-----------------------------------------
-- Scenes
-----------------------------------------
function scene:create( event )
	...
	local options = { score=event.params.score }
	local results = composer.loadScene( "GameMode.categorize_2", true, options )
	...
	local levelText = display.newText( mainGroup, "Nivel 1 completado!", display.contentCenterX, display.contentCenterY - 10, "Fonts/FORTE.TTF", 40 )
...
function scene:hide( event )
	...
	elseif( phase == "did" )then
		...
		composer.hideOverlay( "Scenes.Overlay.categorize_1_win_es" )
	end
end
--scenes listeners
